...
public class VeraPDFService {
    ...
    private PDFVerificationResult convertToVerificationResult(ValidationResult result) {
        ...
        for (TestAssertion assertion : assertions) {
            ...
            if (status == TestAssertion.Status.FAILED) {
                boolean isWarning = isWarningLevel(assertion);
    ...
    private boolean isWarningLevel(TestAssertion assertion) {
        // Currently treat all FAILED assertions as errors
        // In the future, this can be enhanced to detect warnings based on:
        // - Specific rule IDs that are known to be informational
        // - Error severity metadata if available
        // - Specific clauses or test numbers that are non-critical

        // For now, return false to treat all failures as errors
        // This maintains backward compatibility and strict validation
        return false;
    }
    ...
    private String getStandardName(PDFAFlavour flavour) {
        ...
        // PDF/A standards
        if (!id.isEmpty() && id.charAt(0) == '1' || !id.isEmpty() && id.charAt(0) == '2' || !id.isEmpty() && id.charAt(0) == '3' || !id.isEmpty() && id.charAt(0) == '4') {
