...
public class FileRunEventStore {
    ...
    public List<FileRunEvent> list(
            Long teamId, FileRunEventStatus status, boolean closed, String kindId, String actor, int limit) {
        Pageable page = PageRequest.of(0, Math.max(1, limit));
        List<FileRunEventStatus> statuses = closed ? FileRunEventStatus.closed() : FileRunEventStatus.open();
