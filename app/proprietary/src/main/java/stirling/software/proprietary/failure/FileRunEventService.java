...
public class FileRunEventService {
    ...
    public List<FileRunEvent> report(EditorFailureReport report) {
        ...
        List<String> fileIds = report.fileIds().stream()
    ...
    private FileRunEvent recordReported(FailureKind kind, Long teamId, String actor, String fileId, String detail) {
    ...
    public int forgetFiles(List<String> fileIds) {
        ...
        List<String> named =
                fileIds.stream().filter(id -> id != null && !id.isBlank()).toList();
     ...
     */
    public List<FileRunEvent> list(FileRunEventStatus status, boolean closed, String kindId, int limit) {
        ReadScope scope = readScope();
        if (!scope.permitted()) {
            return List.of();
        }
    ...
    public FileRunEvent dispatch(String eventId, String actionId, Map<String, String> inputs) {
        ...
        if (event.status().terminal()) {
            throw new FailureActionException(
                    FailureActionException.Reason.ALREADY_CLOSED, "Event " + eventId + " is already " + event.status());
        }

        FailureAction action = actionRegistry
                .find(resolvedId)
                .orElseThrow(() -> new FailureActionException(
                        FailureActionException.Reason.ACTION_NOT_RECOGNISED, "No handler for action " + resolvedId));
    ...
    public FileRunEvent resolve(String eventId) {
        ...
        return store.applyStatusOnce(
                event.id(), event.teamId(), FileRunEventStatus.RESOLVED, currentActor(), FileRunEventStatus.open());
    }
    ...
    private FileRunEvent notFound(String eventId) {
        throw new FailureActionException(FailureActionException.Reason.EVENT_NOT_FOUND, "No such event: " + eventId);
    }
    ...
    private static AvailableAction availability(
            FailureKind.OfferedAction offer, boolean closed, boolean unattended, boolean documentless) {
        String reason = disabledReasonFor(offer.audience(), closed, unattended, documentless);
        return new AvailableAction(offer.id(), offer.labelKey(), offer.slot(), reason == null, reason);
    }
    ...
    private static boolean offeredTo(FailureAudience audience, Ownership ownership, boolean reviewsTeam) {
        return switch (audience) {
            case OWNER -> ownership == Ownership.MINE || (ownership == Ownership.UNOWNED && reviewsTeam);
