...
class FileRunEventStoreTest {
    ...
    private RecordFailure failure(FailureKind kind, Long teamId, String fileId, String detail) {
        return new RecordFailure(
                kind, FailureOrigin.POLICY, teamId, "ethan@example.com", "policy-1", "run-1", null, fileId, detail);
    }
    ...
    class Insert {
        ...
        void persistsEveryFieldAndSnapshotsTheRegistryFacets() {
            FileRunEvent event = store.record(failure(FailureKind.INPUT_PASSWORD_PROTECTED, TEAM, "file-1", "locked"));
    ...
    class Rollup {
        ...
        void foldsARepeatIntoTheOpenIncidentInsteadOfInsertingAgain() {
            ...
            FileRunEvent second = store.record(failure(FailureKind.INPUT_PASSWORD_PROTECTED, TEAM, "file-1", "again"));
        ...
        void refreshesTheDetailSoTheNewestDiagnosticIsTheOneOnShow() {
            ...
            FileRunEvent second = store.record(failure(FailureKind.UNKNOWN, TEAM, "file-1", "new message"));
        ...
        void absorbsARepeatOntoADismissedRowWithoutReopeningIt() {
            ...
            FileRunEvent first = store.record(failure(FailureKind.INPUT_PASSWORD_PROTECTED, TEAM, "f", "a"));
            store.applyStatus(first.id(), TEAM, FileRunEventStatus.DISMISSED, "reviewer@example.com");

            FileRunEvent repeat = store.record(failure(FailureKind.INPUT_PASSWORD_PROTECTED, TEAM, "f", "b"));
        ...
        void aRetryOfTheSameRunStillFoldsWhenNoFileIdWasCaptured() {
            ...
            assertThat(repository.rows).hasSize(1);
            assertThat(repository.rows.values().iterator().next().getOccurrences())
                    .isEqualTo(2);
        }
    ...
    class Keys {
        ...
        void areFixedWidthSoAnyLengthOfScopeReferenceFitsTheIndex() {
            ...
            RecordFailure command = failure(FailureKind.INPUT_PASSWORD_PROTECTED, TEAM, longKey, "x");
