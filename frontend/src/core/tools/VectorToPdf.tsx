...
const VectorToPdf = ({ onPreviewFile, onComplete, onError }: BaseToolProps) => {
  ...
  useEffect(() => {
    if (!convertParams.parameters.fromExtension && !convertParams.parameters.toExtension) {
      convertParams.updateParameter('fromExtension', 'eps'); // Default vector format
      convertParams.updateParameter('toExtension', 'pdf');
    }
  }, []);
  ...
  useEffect(() => {
    ...
    if (selectedFiles.length > 0) {
      ...
      if (!['ps', 'eps', 'epsf'].includes(convertParams.parameters.fromExtension)) {
        convertParams.updateParameter('fromExtension', 'eps'); // Default vector format
      }
      convertParams.updateParameter('toExtension', 'pdf');
      if (hasResults) {
      ...
      if (activeFiles.length === 0) {
        ...
        convertParams.updateParameter('fromExtension', 'eps');
        convertParams.updateParameter('toExtension', 'pdf');
      }
    }
  }, [selectedFiles]);
  ...
  const handleThumbnailClick = (file: File) => {
    onPreviewFile?.(file);
    sessionStorage.setItem("previousMode", "vector-to-pdf");
  };
  ...
  return createToolFlow({
    ...
    executeButton: {
      ...
      testId: "vector-to-pdf-convert-button",
    },
    review: {
      isVisible: hasResults,
      ...
      onUndo: handleUndo,
      testId: "vector-to-pdf-conversion-results",
    },
  });
};

VectorToPdf.tool = () => useVectorConvertOperation;

export default VectorToPdf as ToolComponent;
