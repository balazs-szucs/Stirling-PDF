...
const PdfToVector = ({ onPreviewFile, onComplete, onError }: BaseToolProps) => {
  ...
  useEffect(() => {
    if (!convertParams.parameters.fromExtension && !convertParams.parameters.toExtension) {
      convertParams.updateParameter('fromExtension', 'pdf');
      convertParams.updateParameter('toExtension', 'eps'); // Default vector format
    }
  }, []);
  ...
  useEffect(() => {
    ...
    if (selectedFiles.length > 0) {
      ...
      convertParams.updateParameter('fromExtension', 'pdf');
      if (!['eps', 'ps', 'pcl', 'xps'].includes(convertParams.parameters.toExtension)) {
        convertParams.updateParameter('toExtension', 'eps'); // Default vector format
      }
      if (hasResults) {
      ...
      if (activeFiles.length === 0) {
        ...
        convertParams.updateParameter('fromExtension', 'pdf');
        convertParams.updateParameter('toExtension', 'eps');
      }
    }
  }, [selectedFiles]);
  ...
  const handleThumbnailClick = (file: File) => {
    onPreviewFile?.(file);
    sessionStorage.setItem("previousMode", "pdf-to-vector");
  };
  ...
  return createToolFlow({
    ...
    executeButton: {
      ...
      testId: "pdf-to-vector-convert-button",
    },
    review: {
      isVisible: hasResults,
      ...
      onUndo: handleUndo,
      testId: "pdf-to-vector-conversion-results",
    },
  });
};

PdfToVector.tool = () => useVectorConvertOperation;

export default PdfToVector as ToolComponent;
