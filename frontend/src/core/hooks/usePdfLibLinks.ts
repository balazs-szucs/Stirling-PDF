...
// ---------------------------------------------------------------------------
// Module-level cache – avoids re-parsing the same PDF for every page.
// ---------------------------------------------------------------------------
...
function releaseDocument(url: string): void {
  ...
  if (!entry) return;
  // entry is a Promise<CachedDoc> – resolve it and decrement
  entry.then((cached) => {
    cached.refCount--;
...
function resolveDestArray(
  ...
  // Might be a plain page number (rare, non-standard)
...
/**
 * Attempt to resolve a named destination to a page index.
 * Handles both the /Names/Dests name tree and the legacy /Dests dict in the catalog.
 */
  ...
  if (kidsResolved instanceof PDFArray) {
    for (let i = 0; i < kidsResolved.size(); i++) {
      ...
      if (kid instanceof PDFDict) {
        // Check /Limits to skip subtrees that can't contain our name
...
function extractLinksFromPage(
  ...
  for (let i = 0; i < annots.size(); i++) {
    try {
      ...
      // Convert from PDF coords (bottom-left origin) to display coords (top-left origin)
