...
describe("yieldToMain", () => {
  ...
  it("calls scheduler.yield when available on globalThis", async () => {
    ...
    const originalScheduler = (globalThis as unknown as { scheduler?: unknown }).scheduler;
    (globalThis as unknown as { scheduler?: unknown }).scheduler = { yield: yieldFn };

    try {
      await yieldToMain();
      expect(yieldFn).toHaveBeenCalledTimes(1);
    } finally {
      (globalThis as unknown as { scheduler?: unknown }).scheduler = originalScheduler;
    }
  });
